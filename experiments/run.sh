#!/usr/bin/env bash
#
# Run an EDSParser experiment through the xbench harness.
#
# This directory holds only configuration: the specs in experiments/specs/ and
# their hooks. The engine lives in its own project so other tools can use it
# (BIO-FMI does), and is located here rather than vendored.
#
# Usage:
...
#   ./experiments/run.sh merge_mode --tag pilot --dataset covid_20A
#
...
#
# What this does NOT do: build datasets. Fetching assemblies, calling variants
# and subsetting panels stay in experiments/scripts/ — xbench measures binaries,
# it is not a data pipeline. Build the inputs first, then measure them here.
#
# Env:
...
XBENCH_RUNS="${XBENCH_RUNS:-$HOME/Data/experiments/edsparser/runs}"
