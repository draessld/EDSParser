...
run_scenario_variability_sweep() {
    ...
    for var in "${variabilities[@]}"; do
        ...
        bench_log "  [cartesian] runtime=${BENCH_RUNTIME_S}s  memory=${BENCH_PEAK_MEMORY_MB}MB  throughput=${tp_cart}MB/s"
        ...
        bench_log "  [linear]    runtime=${BENCH_RUNTIME_S}s  memory=${BENCH_PEAK_MEMORY_MB}MB  throughput=${tp_lin}MB/s"
    done
}
