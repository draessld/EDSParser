...
test_basic_generation() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" -o "$TMPDIR/patterns.txt" -n 5 -l 4
    ...
    assert_not_empty "$TMPDIR/patterns.txt" "output patterns file not empty" || return 1
}

test_correct_line_count() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" -o "$TMPDIR/count.txt" -n 10 -l 4
...
test_patterns_use_acgt_alphabet() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" -o "$TMPDIR/alpha.txt" -n 5 -l 4
...
test_default_count() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" -o "$TMPDIR/default.txt"
...
test_single_pattern() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" -o "$TMPDIR/single.txt" -n 1 -l 4
...
test_missing_output_fails() {
    "$TOOL" -i "$DATA_DIR/eds/simple.eds" 2>/dev/null
    local code=$?
