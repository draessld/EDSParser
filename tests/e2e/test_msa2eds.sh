...
test_basic_conversion() {
    "$TOOL" -i "$DATA_DIR/msa/small.msa" -o "$TMPDIR/out.eds" -s "$TMPDIR/out.seds"
...
test_sources_file_created() {
    "$TOOL" -i "$DATA_DIR/msa/small.msa" -o "$TMPDIR/src.eds" -s "$TMPDIR/src.seds"
...
test_eds_format_valid() {
    "$TOOL" -i "$DATA_DIR/msa/small.msa" -o "$TMPDIR/fmt.eds"
    local content
...
test_leds_with_context_length() {
    "$TOOL" -i "$DATA_DIR/msa/small.msa" -o "$TMPDIR/out.leds" -l 3
...
test_default_output_path() {
    ...
    cp "$DATA_DIR/msa/small.msa" "$input"
    "$TOOL" -i "$input"
