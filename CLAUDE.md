...
- `eds2leds`: Transform EDS to l-EDS with linear or cartesian merging
  ...
  - **Path count dominates linear-merge memory** (measured 2026-08-02): on identical input (yeast chromosome1, 230 kb, 35k variants) 1011 samples was killed at a 20 GB cap inside the first merge batch while 50 samples peaked at **43.7 MB in 0.34 s** — a 20x cut in paths bought ~500x in memory. Genome size is *not* the driver: yeast is denser per bp than human (1 variant/7 bp vs 1/38) with longer merge chains. Cause is the sample-level path model (TODO 9c). `experiments/scripts/make_sample_subset.sh` + `run_subset_dataset.sh` build seeded, nested sample subsets and run them under a kernel-enforced `MemoryMax` scope; see TODO.md for the planned path-count experiment and the options for fixing it.
